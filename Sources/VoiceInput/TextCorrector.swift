...
final class TextCorrector {
    ...
    ///
    /// 【排版】里分段那条是两轮实测校准出来的，**改之前先看这段历史**：
    /// 1. 只说"按语义分段换行" → 模型一个换行都不给，159 字口述堆成一整段。
    /// 2. 加"宁可多分一段，也不要堆成一坨" → 摆到另一个极端，每句话都换行，
    ///    同一件事的追问确认也被拆开。
    /// 3. 改成"默认整段不分" → 又摆回不分，连"做完功能"+"订高铁票"这种明显两件事都不分。
    /// 4. 现在这版：给**唯一判据**（换了另一件不相关的事）+ **正反例并给**。
    ///    单方向施压一定会摆过头，必须两个方向同时标定边界。
    ///
    /// ⚠️ 反例的措辞有讲究：它原本以「另外，」开头，而「另外」正是真·话题切换的信号词，
    /// 结果模型学成"看到另外也别分"，D/E 这类明显两件事全不分了。反例开头不能带切换信号词。
    static func systemPrompt(removeFillers: Bool) -> String {
        ...
        【排版】
        - 句内按语义补齐逗号、顿号等标点。
        - **分段**：分段的**唯一理由**是「一件事说完了，开始说另一件不相关的事」。在两件事的交界处换行。
          同一件事的展开一律留在同一段，包括举例、罗列、补充、追问、确认，哪怕说了三五句、中间有多个问号。
          - **不分**（整段都在问同一件事）：「我想再跟你确认一下，stage0 这个阶段会产生的产物都有什么？是一、产物存档，二、母题卡，三、禁用清单，是这些吗？那么评审报告该放到哪里呢」
          - **要分**（两件事互不相关，交界处换行）：「先帮我把专名词典的导入做完」换行「我明天要去北京出差，帮我看一下有没有早上的高铁票」
          - **要分**（三件事，两处换行）：「把昨天崩溃的日志发我看看」换行「记得把测试补上」换行「下周的评审会帮我约一下时间」
        - **段与段之间只用一个换行，不要留空行。**
